...
- **Character cap is user-tunable (10/20/30/40/50, default 20)** and persisted as `jobs.character_cap` (migration `0003`). The prompt template uses a `{CHAR_CAP}` placeholder; `_render_system_prompt` substitutes via `str.replace` (NOT `str.format`, because the prompt's TypeScript schema block has literal `{` / `}`). Cache key includes the cap — a cap=10 request will never serve a cap=50 cached map.
...
- **Spoiler acknowledgement is temporarily pre-checked.** `Home.tsx` defaults `spoilerAcknowledged` to `true` during dev iteration. Has a comment marker `Re-default to false before any public deploy`; Planka card filed. Backend still validates `acknowledged_spoilers: true` — it's just the frontend friction that's gone.
