...
- **Edge handle selection is computed at build time, not live.** `src/layout/layout.ts` picks `sourceHandle`/`targetHandle` once from initial `nodeCenters` map. If a user drags a faction group across the canvas, the handle pair stays stale — known follow-up. Reset layout button is the workaround.
